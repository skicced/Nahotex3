...
$(function() {
    ...
    $("form#form").on("submit", function() {
        ...
        if ($(".requiredField").each(function() {
                if ("" === jQuery.trim($(this).val())) $(this).prev("label").text(), $(this).parent().append('<span class="error">Üres mező</span>'), $(this).addClass(
                    "inputError"), s = !0;
                else if ($(this).hasClass("email")) {
                    var r = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;
                    r.test(jQuery.trim($(this).val())) || ($(this).prev("label").text(), $(this).parent().append('<span class="error">Hibás email cím</span>'), $(this).addClass(
                        "inputError"), s = !0);
                }
            }), !s) {
            ...
            $.post($(this).attr("action"), r, function() {
                $("form#form").slideUp("fast", function() {
                    $(this).before('<div class="success">Köszönjük, hogy felvette velünk a kapcsolatot.</div>');
                });
            });
        }
        return !1;
